...
void test_banco_add_and_get_quadra(void) {
    banco_addQuadra("12345", 10.0, 20.0, 100.0, 50.0);
...
void test_banco_quadra_style(void) {
    banco_setQuadraStyle("red", "blue", "2.5px");
    ...
    TEST_ASSERT_EQUAL_STRING("2.5px", quadra_get_stroke_width(q));
...
void test_banco_get_quadra_residents(void) {
    ...
    TEST_ASSERT_EQUAL_INT(3, counts[0]);  // total
    TEST_ASSERT_EQUAL_INT(1, counts[1]);  // Norte
    TEST_ASSERT_EQUAL_INT(1, counts[2]);  // Sul
    TEST_ASSERT_EQUAL_INT(1, counts[3]);  // Leste
    TEST_ASSERT_EQUAL_INT(0, counts[4]);  // Oeste
}

void test_banco_get_quadra_residents_with_west(void) {
    banco_addQuadra("cepWest", 0, 0, 100, 100);
    banco_addPessoa("w1", "West", "One", 'M', "01/01/2000");
    banco_addPessoa("w2", "West", "Two", 'M', "02/02/2000");
    
    banco_morar("w1", "cepWest", 'W', 10, "");  // W deve ser convertido para O
    banco_morar("w2", "cepWest", 'O', 20, "");
    
    int counts[5];
    banco_getQuadraResidents("cepWest", counts);
    
    TEST_ASSERT_EQUAL_INT(2, counts[0]);  // total
    TEST_ASSERT_EQUAL_INT(0, counts[1]);  // Norte
    TEST_ASSERT_EQUAL_INT(0, counts[2]);  // Sul
    TEST_ASSERT_EQUAL_INT(0, counts[3]);  // Leste
    TEST_ASSERT_EQUAL_INT(2, counts[4]);  // Oeste (W e O convertidos)
}
...
void test_banco_remove_quadra_affects_residents(void) {
    ...
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_EQUAL_INT(0, pessoa_eh_morador(p));
    TEST_ASSERT_EQUAL_STRING("", pessoa_get_cep(p));
    free(p);
}
...
void test_banco_get_pessoa_index(void) {
    ...
    Pessoa *p3 = banco_getPessoaIndex(99);
...
int main(void) {
    ...
    RUN_TEST(test_banco_get_quadra_residents_with_west);
